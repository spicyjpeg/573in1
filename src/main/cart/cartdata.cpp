...
void IdentifierSet::updateTraceID(
	...
	switch (type) {
		...
		case TID_82_LITTLE_ENDIAN:
			...
			LOG_CART("prefix=0x82, checksum=0x%04x", checksum);
			break;
	}
...
bool BasicHeader::validateChecksum(bool invert) const {
	...
	if (value != checksum) {
		LOG_CART("mismatch, exp=0x%02x, got=0x%02x", value, checksum);
		return false;
	}
...
bool ExtendedHeader::validateChecksum(bool invert) const {
	...
	if (value != checksum) {
		LOG_CART("mismatch, exp=0x%04x, got=0x%04x", value, checksum);
		return false;
	}
...
bool CartParser::validate(void) {
	...
	if (getRegion(region) < REGION_MIN_LENGTH) {
		LOG_CART("region is too short: %s", region);
		return false;
	}
	if (!isValidRegion(region)) {
		LOG_CART("invalid region: %s", region);
		return false;
	}
	...
	if (id) {
		...
		if (flags) {
			LOG_CART("flags do not match, value=%02x", flags);
			return false;
		}
	}
#endif
...
CartParser *newCartParser(CartDump &dump) {
	...
	for (int i = util::countOf(_KNOWN_CART_FORMATS) - 1; i >= 0; i--) {
		...
		LOG_CART("trying as %s", format.name);
		if (parser->validate())
			return parser;
	...
	LOG_CART("unrecognized data format");
	return nullptr;
}
...
template<typename T> const T *DB<T>::lookup(
	...
	while (low <= high) {
		...
		if (!diff) {
			LOG_CART("%s %s found", code, region);
			return entry;
		}
	...
	LOG_CART("%s %s not found", code, region);
	return nullptr;
}
